...
static uint32_t DPImm(char **tokens, int numTokens) {
  ...
  uint32_t opc;
  uint32_t operand = 0;
  uint32_t rd = registerIndexAsInt(tokens[1]);
  ...
  if (strcmp("movk", opcode) == 0 || strcmp("movn", opcode) == 0 || strcmp("movz", opcode) == 0) { // wide move
    ...
    uint32_t rn = registerIndexAsInt(tokens[2]);
...
static uint32_t DPReg(char **tokens, int numTokens) {
  ...
  uint32_t M;
  uint32_t opr;
  uint32_t rm = registerIndexAsInt(tokens[3]);
  uint32_t rn = registerIndexAsInt(tokens[2]);
  uint32_t rd = registerIndexAsInt(tokens[1]);
  uint32_t operand = 0;
  ...
  if (strcmp(opcode, "madd") == 0 || strcmp(opcode, "msub") == 0) {
    ...
    operand |= registerIndexAsInt(tokens[4]);
    M = 1;
    opr = 8;
