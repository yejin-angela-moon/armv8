...
static uint32_t conditional(symbol_table_row *symbol_table, char **tokens, int cond, uint32_t currAddress) {
    ...
    if ((strcmp(tokens[1], "0x0") != 0 || strcmp(tokens[1], "0") != 0) && strtol(tokens[1], NULL, 0) == 0) {
        address = (int) findAddressOfLabel(tokens[1], symbol_table);
    } else {
        address = (int) strtol(tokens[1], NULL, 16);
    }
...
uint32_t B(symbol_table_row *symbol_table, char **tokens, uint32_t currAddress) {
    ...
    if (strcmp(tokens[0], "b") == 0) {
        ...
        // add the register << 5
        uint32_t reg = registerToBinary(tokens[1]);
        instruction += reg << 5;

    } else {
        char *condStr = strtok(tokens[0], "b.");
        if (strcmp(condStr, "eq") == 0) {
            return conditional(symbol_table, tokens, 0, currAddress);
        } else if (strcmp(condStr, "ne") == 0) {
            return conditional(symbol_table, tokens, 1, currAddress);
        } else if (strcmp(condStr, "ge") == 0) {
            return conditional(symbol_table, tokens, 0xA, currAddress);
        } else if (strcmp(condStr, "lt") == 0) {
            return conditional(symbol_table, tokens, 0xB, currAddress);
        } else if (strcmp(condStr, "gt") == 0) {
            return conditional(symbol_table, tokens, 0xC, currAddress);
        } else if (strcmp(condStr, "le") == 0) {
            return conditional(symbol_table, tokens, 0xD, currAddress);
        } else if (strcmp(condStr, "al") == 0) {
            return conditional(symbol_table, tokens, 0xE, currAddress);
        }
    }
